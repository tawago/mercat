//! Attributable bundle tier of the report-only structural audit.
//!
...
//! whether recorded bundle identity agrees with the membership derivation,
...
fn stampAvailable(s: sketch.Sketch, c: *counts.Counts) bool {
    switch (s.bundle_stamp_state) {
        .unattempted => c.u_bundle_stamp_unattempted += 1,
        .out_of_memory => c.u_bundle_stamp_oom += 1,
        .rail_invariant => c.u_bundle_stamp_rail_invariant += 1,
        .complete => {
            if (ledger.rosterNumbered(s.bundle_sets)) {
                c.n_bundle_stamp_complete += 1;
                return true;
            }
            c.u_bundle_roster_inconsistent += 1;
        },
    }
    return false;
}
...
fn compareDetail(detail: u8, recorded: ?bool, c: *counts.Counts) void {
    ...
    const claim: bool = switch (kind) {
        ...
        .merged_untested => {
            c.u_bundle_detail_untested += 1;
            return;
        },
    };
    const identity = recorded orelse {
        c.u_bundle_detail_identity_unavailable += 1;
        return;
    };
    c.n_bundle_details_compared += 1;
    if (claim == identity) {
        c.m_bundle_detail_agreed += 1;
    } else {
        c.u_bundle_detail_disagreed += 1;
    }
}
...
pub fn check(v: cell.View, s: sketch.Sketch, c: *counts.Counts) void {
    ...
    if (aux.state != .complete) {
        c.u_bundle_record_aux_unavailable += 1;
        return;
    }

    for (v.auxRecords()) |r| {
        if (r.kind != .carrier) continue;
        c.n_bundle_carrier_records += 1;
        const t = v.atIndex(r.cell) orelse {
            c.u_bundle_record_owner_absent += 1;
            continue;
        };
        ...
        if (r.value == held) {
            c.u_bundle_record_restates_owner += 1;
            continue;
        }

        c.n_bundle_carrier_pairs += 1;
        var recorded: ?bool = null;
        if (identity_available) {
            const x = r.cell % v.width();
            const y = r.cell / v.width();
            const at: ledger.BundleCell = .{ .x = @intCast(x), .y = @intCast(y) };
            const identity = ledger.bundlesAgree(s.bundle_sets, held, r.value, at);
            recorded = identity;
            const derived = ledger.derivedSameBundle(s.bundles, s.bundle_sets, held, r.value, at);
            c.n_bundle_pairs_compared += 1;
            if (identity == derived) {
                c.m_bundle_identity_agreed += 1;
            } else {
                c.u_bundle_identity_disagreed += 1;
            }
        } else {
            c.u_bundle_identity_unavailable += 1;
        }
        compareDetail(r.detail, recorded, c);
    }

    if (c.n_bundle_carrier_records == 0) c.u_bundle_population_absent += 1;
}
