...
//! rails of one inter-layer gap.
//!
//! `fan_lanes.zig` decides HOW MANY rail rows a gap needs and which rail is
//! entitled to one; it does not decide WHICH row each rail takes, and for a
//! star decomposition that choice is load-bearing.
//!
//! A rail's stem meets its crossbar at one cell — the stem column on the rail
//! row. Every OTHER rail in the gap runs its taps as verticals from the peer
...
//! rail and whose corner belongs to another. Neither run can be traced through
//! it, and the reach oracle reports it (`reach_unknown_continuation`).
//!
//! The conflict is one of ORDER, not of position. A fan-IN rail's taps occupy
//! the rows between the sources and its own rail; a rail whose stem column is
//! one of them only has to sit BELOW that rail for its junction to fall outside
//! the tap's span. So this pass reads the placed columns of every rail in a
...
/// One rail-producing rail as this pass sees it: the gap it crosses, the lane
...
/// The stem column of a resolved rail — the same expression `fan_rail.build`
...
/// The tap columns of a resolved rail, in peer order (`fan_rail.build`'s).
...
/// Re-order `rails`' lane indices, per gap, so that no rail's stem junction
...
pub fn reorder(a: std.mem.Allocator, rails: []Rail) error{OutOfMemory}!void {
    ...
    for (gap_seen.items) |gap| try reorderGap(a, rails, gap);
}

fn reorderGap(a: std.mem.Allocator, rails: []Rail, gap: u32) error{OutOfMemory}!void {
    ...
    for (0..n) |k| {
        for (0..n) |j| {
            if (k == j) continue;
            if (!contains(rails[idx.items[j]].tap_xs, rails[idx.items[k]].stem_x)) continue;
            before[k * n + j] = true;
            any = true;
        }
    }
