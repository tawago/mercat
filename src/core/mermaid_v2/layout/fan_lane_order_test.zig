...
fn lanesOf(trunks: []const order.Trunk, out: []u32) void {
    for (trunks, out) |t, *slot| slot.* = t.lane;
}

test "a gap with no shared stem column keeps the packer's lanes" {
    var t0_taps = [_]i32{ 1, 20 };
    var t1_taps = [_]i32{ 3, 22 };
    var trunks = [_]order.Trunk{
        .{ .gap = 0, .lane = 0, .fan_in = true, .stem_x = 10, .tap_xs = &t0_taps },
        .{ .gap = 0, .lane = 1, .fan_in = true, .stem_x = 30, .tap_xs = &t1_taps },
    };
    try order.reorder(testing.allocator, &trunks);
    try testing.expectEqual(@as(u32, 0), trunks[0].lane);
    try testing.expectEqual(@as(u32, 1), trunks[1].lane);
}

test "a stem crossed by a foreign tap is ordered below it" {
    // Trunk 1's stem column (56) is one of trunk 0's tap columns. Trunk 0's
    // taps run from the sources down to trunk 0's own rail, so trunk 1's
    ...
    try order.reorder(testing.allocator, &trunks);
    try testing.expectEqual(@as(u32, 0), trunks[1].lane);
    try testing.expectEqual(@as(u32, 1), trunks[0].lane);
}

test "the directed all-to-all's three arrival trunks get a clearing order" {
    // The K(3,3) shape this pass exists for: stems 10/33/56, taps staggered
    // by two columns per trunk. Trunk 0's and trunk 2's stems both sit on
    // trunk 1's tap columns, so trunk 1 must take the FAR row and the other
    ...
    lanesOf(&trunks, &lanes);
    // A permutation, and trunk 1 is strictly beyond both of the others.
...
test "trunks in different gaps are ordered independently" {
    var g0a = [_]i32{ 10, 33 };
    var g0b = [_]i32{ 12, 35 };
    var g1a = [_]i32{ 4, 9 };
    var trunks = [_]order.Trunk{
    ...
    try order.reorder(testing.allocator, &trunks);
    // Gap 1's single trunk is untouched; gap 0 re-ordered on its own.
    try testing.expectEqual(@as(u32, 0), trunks[2].lane);
    try testing.expect(trunks[0].lane != trunks[1].lane);
}
