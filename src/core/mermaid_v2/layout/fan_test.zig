...
test "a fan-OUT with three labeled members reserves the same gap rows as one with a single labeled member" {
    const a = testing.allocator;
    var arena = std.heap.ArenaAllocator.init(a);
    defer arena.deinit();
    const aa = arena.allocator();
    ...
    const rows_one = try fan.extraRowsPerGap(aa, lg, &one);
    try testing.expectEqual(@as(u32, 1 + fan.LABEL_RUN_EXTRA_ROWS), rows_one[0]);
    ...
    const rows_three = try fan.extraRowsPerGap(aa, lg, &three);
    try testing.expectEqual(rows_one[0], rows_three[0]);
}
