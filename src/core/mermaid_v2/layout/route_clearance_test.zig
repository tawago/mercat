...
test "a decorated terminal's lateral neighbours refuse a foreign bend and a foreign run toward the head, admit a parallel through-run" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();
    const placements = [_]sk.NodePlacement{
    ...
    const bend = [_]sk.Point{ .{ .x = 8, .y = 9 }, .{ .x = 3, .y = 9 }, .{ .x = 3, .y = 5 } };
    try std.testing.expect(try clearance.conflictsReservedTerminals(a, 1, &bend, &placements, &decorated, .{}));
    ...
    try std.testing.expect(!try clearance.conflictsReservedTerminals(a, 1, &bend, &placements, &plain, .{}));
