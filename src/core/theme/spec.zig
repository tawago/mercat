...
pub const TableStyle = enum { grid, rounded };
...
pub const ThemeSpec = struct {
    ...
    /// `slots` covering only the code-token slots that differ. This axis is
    /// consumed ONLY by the legacy `theme.palette(_, .classic)` API (used by the
    /// export/PNG paths and their tests); the resolve/render/`--dump-theme`
    /// pipeline always uses the default variant (`slots`). Kept here so
    /// `presets.zig` remains the single source of truth for both variants.
