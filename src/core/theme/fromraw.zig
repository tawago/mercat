...
fn applyGlyphKv(alloc: std.mem.Allocator, g: *spec.GlyphSet, kv: loadfile.RawKV, diag: *Collector) void {
    ...
    if (std.mem.eql(u8, k, "ordered_prefix")) {
        ...
        if (std.mem.eql(u8, v, "grid")) g.table_style = .grid else if (std.mem.eql(u8, v, "rounded")) g.table_style = .rounded else diag.warnFmt(.unknown_key, "invalid table_style '{s}'", .{v});
    } else {
