...
pub const Registry = struct {
    ...
    /// Resolve `name` (+ optional inline overrides) into a concrete theme,
    /// reporting every soft failure to `diag`. Always returns a usable theme.
    pub fn resolve(
        self: *const Registry,
        name: []const u8,
        ...
        var folded = foldSpecs(self.alloc, chain, inline_overrides, diag);
        const base_light = std.mem.eql(u8, chain[0].name, "light");
        return bake(&folded, base_light, diag);
    }

    /// Fold `name`'s extends chain into one flattened `ThemeSpec` (no inline
    /// overrides, no palette bake). Returns null for an unknown name (reporting
    /// `unknown_theme`) — used by `--dump-theme`, which must fail hard rather
    /// than silently fall back to dark. The returned spec borrows string slices
    /// from the registry's specs, so it must not outlive them.
    pub fn foldedSpec(self: *const Registry, name: []const u8, diag: *Collector) ?ThemeSpec {
        ...
        var chain_buf: [max_chain]*const ThemeSpec = undefined;
        const chain = self.buildChain(leaf, &chain_buf, diag);
        return foldSpecs(self.alloc, chain, null, diag);
    }
...
/// Fold a root→leaf chain (plus inline overrides) into one dense-ish spec.
/// Later entries override earlier: absent field inherits, present field wins
/// (a present `""` prefix/icon deliberately clears — see bake).
fn foldSpecs(
    alloc: std.mem.Allocator,
...
/// Convenience: resolve a built-in preset with no overrides. Built-in glyphs
/// and prefixes are static data, so the returned theme owns no allocations and
/// needs no deinit; `alloc` backs only a throwaway diagnostics collector.
pub fn builtinResolved(alloc: std.mem.Allocator, name: []const u8) ResolvedTheme {
    ...
    defer d.deinit();
    return reg.resolve(name, null, &d) catch unreachable;
}
...
// EVERY one of the 35 palette slots. If any slot in `presets.dark`/`light`
...
test "resolve(dark/light) == theme.palette default variant across all 35 slots" {
    ...
    for (cases) |c| {
        const r = try reg.resolve(c.name, null, &diag);
...
test "resolve dark yields a full palette + total decor" {
    ...
    const r = try reg.resolve("dark", null, &diag);
    try testing.expectEqual(@as(usize, 0), diag.count());
...
test "dark/light bake to the render/decor legacy Decor (goldens safety net)" {
    ...
    for ([_][]const u8{ "dark", "light" }) |name| {
        var diag = Collector.init(testing.allocator);
        defer diag.deinit();
        const r = try reg.resolve(name, null, &diag);
...
test "every built-in preset resolves with zero diagnostics" {
    ...
    for (presets.ALL) |p| {
        var diag = Collector.init(testing.allocator);
        defer diag.deinit();
        const r = try reg.resolve(p.name, null, &diag);
        try testing.expectEqual(@as(usize, 0), diag.count());
...
test "ansi preset bakes ansi16-typed slot colors" {
    ...
    var diag = Collector.init(testing.allocator);
    defer diag.deinit();
    const r = try reg.resolve("ansi", null, &diag);
...
test "light preset bakes to the light base palette" {
    ...
    var diag = Collector.init(testing.allocator);
    defer diag.deinit();
    const r = try reg.resolve("light", null, &diag);
...
test "unknown theme name falls back to dark with a diagnostic" {
    ...
    const r = try reg.resolve("nope", null, &diag);
...
test "inline override changes a slot color" {
    ...
    const r = try reg.resolve("dark", raw, &diag);
...
test "sparse merge: child sets fg only, inherits prefix from base" {
    ...
    var folded = foldSpecs(testing.allocator, &chain, null, &diag);
...
test "canvas merges through the extends chain (absent inherits, present wins)" {
    ...
    const folded_inherit = foldSpecs(testing.allocator, &chain_inherit, null, &diag);
    ...
    const folded_override = foldSpecs(testing.allocator, &chain_override, null, &diag);
...
test "buildChain detects a cycle and still resolves" {
    ...
    const r = try reg.resolve("acyc", null, &diag);
...
test "missing extends target reports and falls back to dark" {
    ...
    const r = try reg.resolve("orphan", null, &diag);
...
test "extends chain depth >= 2 folds correctly" {
    ...
    const r = try reg.resolve("child", null, &diag);
    try testing.expectEqual(@as(usize, 0), diag.count());
...
test "user file as named theme resolves via specFromRaw" {
    ...
    const r = try reg.resolve("usertheme", null, &diag);
...
test "user file as extends= target folds into the chain" {
    ...
    const r = try reg.resolve("userleaf", null, &diag);
    try testing.expectEqual(color.rgb(0x0a, 0x0b, 0x0c), r.palette.body.fg);
    try testing.expectEqual(color.idx(7), r.palette.link.fg);
}

test "user file extends a built-in preset: non-overridden slots equal the preset" {
    ...
    const r = try reg.resolve("tmptest", null, &diag);
    try testing.expectEqual(@as(usize, 0), diag.count());

    const drac = try reg.resolve("dracula", null, &diag);
...
test "user file with missing/cyclic root extends still reports diagnostics" {
    ...
    _ = try reg1.resolve("orphanfile", null, &d1);
    ...
    _ = try reg2.resolve("filea", null, &d2);
