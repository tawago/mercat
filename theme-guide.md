...
tip: open me with `mercat --style <preset> theme-guide.md`
---
...
Header cells use `[theme.table_header]`; the border rules use
`[theme.table_border]`. Both leave a preset that never sets them borrowing the
same tokens the un-themed output used (`body` for headers, `muted` for
borders), so parity holds until a theme opts in.

The border **glyph set** is picked by `[theme.glyphs] table_style`, which
accepts `grid` (default `─│┼`), `heavy` (`━┃╋`), `double` (`═║╬`), `ascii`
(`-|+`), and `rounded` (grid rules with `╭╮╰╯` corners — markview uses this).
...
`[theme.hr]` controls the color; the glyph and fill mode come from
`[theme.glyphs]` (`hr_glyph`, and `hr_mode` = full-width, below, or a
fixed count like glamour's eight dashes via `fixed`, clamped to the terminal
width):
...
(ansi), or a full-width block with a language chip (markview). A fenced block's
language banner is colored by `[theme.code_fence_banner]`.
...
`bullet`, `ordered`, `task_on`, `task_off`, `list_item`, `table_border`,
`table_header`, `hr`, `code_fence_banner`.

The last four — `table_border`, `table_header`, `hr`, `code_fence_banner` —
are structural color slots. A theme that leaves them unset inherits the
borrowed tokens the un-themed output used (`table_border`/`hr`/
`code_fence_banner` → `muted`, `table_header` → `body`), so setting them is
purely opt-in.
